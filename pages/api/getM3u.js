...
const getUserChanDetails = async () => {
    ...
    try {
        const response = await fetch("https://lust.toxicify.pro/api/8");
        const data = await response.json();
        const mpdUrl = data.mpd;

        const hmacMatch = mpdUrl.match(/hmac=([^&]*)/);
        if (hmacMatch && hmacMatch[1]) {
            hmacValue = hmacMatch[1];
        } else {
            throw new Error('hmac value not found in mpd URL');
        }
    } catch (error) {
